...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full backdrop-blur-xl bg-black/85 border-b border-slate-800/80 shadow-md shadow-black/40 transition-all duration-300">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <div className="relative">
              ...
              <span className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-slate-950 flex items-center justify-center shadow-xs">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold text-xl tracking-tight bg-gradient-to-r from-white via-sky-200 to-sky-400 bg-clip-text text-transparent group-hover:to-sky-300 transition-colors">
                  {profile.name}
                </span>
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-500/20 text-sky-400 border border-sky-500/30">
                  PRO
                </span>
              </div>
              <span className="text-xs font-semibold text-slate-400 flex items-center gap-1">
                <span className="text-sky-400 font-medium">Graphic Designer & Video Editor</span>
              </span>
            </div>
          </a>
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            {navLinks.map((link) => (
              <a
                ...
                className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-sky-400 hover:bg-slate-900/80 transition-all"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-2.5">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-[#1877F2] bg-slate-900 hover:bg-[#1877F2]/20 border border-[#1877F2]/30 hover:border-[#1877F2]/50 rounded-xl shadow-2xs transition-all hover:scale-102"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-sky-400 bg-slate-900 hover:bg-slate-800 border border-slate-700/80 rounded-xl shadow-2xs transition-all hover:scale-102"
          ...
          <div className="flex md:hidden items-center gap-2">
            <button
              ...
              type="button"
              className="p-2 rounded-xl text-slate-300 hover:text-sky-400 hover:bg-slate-900 focus:outline-none"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-800 bg-slate-950/95 backdrop-blur-xl px-5 pt-3 pb-6 space-y-2 shadow-2xl animate-fadeIn">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              onClick={() => setMobileMenuOpen(false)}
              className="block px-3.5 py-2.5 rounded-xl text-base font-semibold text-slate-200 hover:text-sky-400 hover:bg-slate-900"
            >
              {link.label}
            </a>
          ))}
          <div className="pt-4 border-t border-slate-800 flex flex-col gap-2.5">
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-sky-300 bg-slate-900/90 border border-slate-700/80 rounded-xl"
            >
