...
export const Showcase: React.FC<ShowcaseProps> = ({
  ...
  return (
    <section id="showcase" className="py-20 sm:py-24 bg-gradient-to-b from-white via-sky-50/40 to-slate-50 relative">
      {/* Background visual accents */}
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-sky-300 to-transparent" />
      <div
        className="pointer-events-none absolute top-32 right-6 w-96 h-96 bg-sky-200/30 rounded-full blur-3xl -z-10"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute bottom-20 left-6 w-96 h-96 bg-cyan-100/40 rounded-full blur-3xl -z-10"
        aria-hidden="true"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-100/80 border border-sky-200 text-sky-800 text-xs font-bold uppercase tracking-wider mb-3">
              <Layers className="w-4 h-4 text-sky-600" />
              <span>Creative Portfolio Showcase</span>
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight">
              Graphic Design &{' '}
              <span className="bg-gradient-to-r from-purple-600 via-sky-600 to-blue-600 bg-clip-text text-transparent">
                Video Editing Works
              </span>
            </h2>
            <p className="text-slate-600 mt-3 max-w-2xl text-base sm:text-lg leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            {onResetProjects && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:text-sky-700 bg-white hover:bg-sky-50 border border-slate-200 hover:border-sky-300 shadow-2xs transition-all"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-10 pb-4 border-b border-sky-200/70">
          {/* Category Tabs */}
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-200 flex items-center gap-1.5 ${
                    active
                      ...
                      : 'bg-white text-slate-600 hover:text-sky-700 hover:bg-sky-50 border border-slate-200 shadow-2xs'
                  }`}
                >
                  <span>{cat.icon}</span>
                  <span>{cat.label}</span>
          ...
          <div className="relative min-w-[260px] sm:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2.5 text-sm bg-white border border-sky-200 rounded-xl focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 shadow-2xs transition-all text-slate-800 placeholder-slate-400"
            />
            {searchQuery && (
              <button
        ...
        {filteredProjects.length === 0 ? (
          <div className="py-20 text-center bg-white rounded-3xl border-2 border-dashed border-sky-200 shadow-xs">
            <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-sky-50 border border-sky-200 flex items-center justify-center text-sky-600">
            ...
            <button
              ...
              className="mt-5 px-5 py-2 text-xs font-bold text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-200 rounded-xl transition-colors"
            >
              Reset Filters
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map((project) => (
              <div
                ...
                className="group flex flex-col bg-white rounded-3xl overflow-hidden border-2 border-sky-100/90 shadow-sm hover:shadow-2xl hover:shadow-sky-500/15 hover:border-sky-300 transition-all duration-300 transform hover:-translate-y-1.5 cursor-pointer"
              ...
              >
                ...
                <div className="relative h-56 w-full overflow-hidden bg-slate-100">
                  ...
                  {project.videoUrl && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className={`w-13 h-13 rounded-2xl bg-black/60 backdrop-blur-md border border-white/30 flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-all ${
                  ...
                  <div className="absolute top-3.5 left-3.5">
                    <span className={`px-3 py-1 text-xs font-extrabold tracking-wide uppercase rounded-xl shadow-xs border backdrop-blur-md ${
                      project.category === 'video'
                        ...
                        : 'bg-white/95 text-sky-800 border-sky-100'
                    }`}>
                  ...
                  <div className="absolute top-3.5 right-3.5 flex items-center gap-1.5">
                    ...
                    {onEditProject && (
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-black/60 hover:bg-sky-600 text-white backdrop-blur-md transition-colors shadow-xs"
                        title="Edit Project"
                      >
                    ...
                    {onDeleteCustomProject && (
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-rose-500/90 hover:bg-rose-600 text-white backdrop-blur-xs transition-colors shadow-xs"
                  ...
                  {project.metrics && (
                    <div className="absolute bottom-3 left-4 right-4 text-xs font-semibold text-sky-200 truncate flex items-center gap-1.5">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    {/* Project Title */}
                    <h3 className="text-xl font-extrabold text-slate-900 group-hover:text-sky-600 transition-colors line-clamp-1">
                      {project.title}
                    </h3>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {(project.toolsUsed && project.toolsUsed.length > 0 ? project.toolsUsed : project.tags).slice(0, 4).map((tool, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 text-xs font-bold text-sky-800 bg-sky-50/90 border border-sky-200/80 rounded-lg group-hover:bg-sky-100/90 transition-colors"
                        >
                          {tool}
                        </span>
                      ))}
                  ...
                  {/* Action Link Buttons */}
                  <div
                    className="mt-6 pt-5 border-t border-sky-100 flex items-center justify-between gap-2.5"
                  ...
                  >
                    ...
                    {onEditProject && (
                      <button
                        ...
                        type="button"
                        className="inline-flex items-center justify-center p-2.5 rounded-xl text-slate-600 hover:text-sky-700 bg-slate-50 hover:bg-sky-50 border border-slate-200 hover:border-sky-300 transition-all hover:scale-105"
                        title="Edit Project"
                      >
